...
7. Reject invalid transitions if consistent with the current lifecycle style.
8. Add a domain exception if invalid delete transitions are possible, following the style of InvalidContentItemArchiveException / InvalidContentItemRestoreException.
9. Update audit fields such as updatedBy/updatedAt if the operation is soft-delete based. If it is hard-delete based, preserve existing deletion conventions.
10. EventPublishingContentItemService must dispatch ContentItemDeletedEvent only after the delegate operation succeeds.
