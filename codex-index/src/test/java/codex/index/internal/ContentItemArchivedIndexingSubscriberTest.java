...
/**
 * Tests for {@link ContentItemArchivedIndexingSubscriber}.
 */
class ContentItemArchivedIndexingSubscriberTest {
    ...
    private ContentItemArchivedIndexingSubscriber subscriber;
    private ContentItemArchivedEvent event;

    @BeforeEach
    void setUp() {
        indexWriter = new RecordingIndexWriter();
        subscriber = new ContentItemArchivedIndexingSubscriber(indexWriter);
        event = new ContentItemArchivedEvent(ITEM_ID, SITE_KEY, CT_KEY, CT_VERSION_ID, ITEM_KEY, ACTOR, NOW);
    }

    @Test
    void rejectsNullIndexWriter() {
        assertThrows(NullPointerException.class, () -> new ContentItemArchivedIndexingSubscriber(null));
    }
