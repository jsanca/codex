...
/**
 * Tests for {@link ContentItemDeletedIndexingSubscriber}.
 */
class ContentItemDeletedIndexingSubscriberTest {
    ...
    private ContentItemDeletedIndexingSubscriber subscriber;
    private ContentItemDeletedEvent event;

    @BeforeEach
    void setUp() {
        indexWriter = new RecordingIndexWriter();
        subscriber = new ContentItemDeletedIndexingSubscriber(indexWriter);
        event = new ContentItemDeletedEvent(ITEM_ID, SITE_KEY, CT_KEY, CT_VERSION_ID, ITEM_KEY, ACTOR, NOW);
    }

    @Test
    void rejectsNullIndexWriter() {
        assertThrows(NullPointerException.class, () -> new ContentItemDeletedIndexingSubscriber(null));
    }
