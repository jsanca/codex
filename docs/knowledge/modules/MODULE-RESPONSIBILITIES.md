...
> **Current status:** all declared Maven/Jigsaw modules exist. `codex-fundamentum`,
> `codex-codex`, `codex-index`, `codex-chronicon`, `codex-concilium`, and `codex-custos`
> contain implemented behavior; the remaining edge modules are skeletons.
...
> **Current boundary note:** Indexing abstractions and subscribers live in `codex-index`.
> `codex-codex` exposes projection reads and lifecycle events, but does not contain index types
> or indexing subscribers.
...
> **Current status:** `ConciliumRuntime.inMemory()` composes the unsecured local runtime and
> `ConciliumRuntime.secured(...)` composes secured Site, ContentType, and ContentItem services.
> `RuntimeSecurityMode` is diagnostic metadata; enforcement is in the exposed service graph.
> ServiceLoader provider and Porta integration remain future work.
...
## Historical Proposal: Nuntius

> `codex-nuntius` is not a declared Maven module. This section preserves an earlier conceptual
> proposal and must not be read as current module documentation.
...
## Historical Proposal: Speculum

> `codex-speculum` is not a declared Maven module. This section preserves an earlier conceptual
> proposal and must not be read as current module documentation.
...
| codex-custos    | Authorization model      | No                    | Domain services          | No       | Yes                      |
...
- Indexing foundation: `IndexDocument`, `IndexWriter`, indexing subscribers, and
  `ContentItemProjectionSource` in `codex-index`
- Cache foundation: `CacheEntry`, `CacheRegion`, `ConcurrentMapCacheRegion`, `RecordingCacheRegion`
  (in `codex-fundamentum`)
- Local event subscribers for indexing projection and Chronicon domain audit
...
1. **`IndexDocumentIdFactory`**: Should this abstraction wait until tenant-aware indexing is
