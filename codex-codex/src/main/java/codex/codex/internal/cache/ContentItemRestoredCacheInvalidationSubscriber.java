 ...
 * {@link ContentItemRestoredEvent} is dispatched.
 *
 * <p>Restoring a {@link codex.codex.api.model.entity.ContentItem} from archive changes its
 * status. The cached item would be stale after this transition. This subscriber evicts it
 * so the next read returns the updated item from the delegate.</p>
 *
 * <p>This subscriber is not wired into runtime yet. Runtime wiring is a separate task.</p>
 */
public final class ContentItemRestoredCacheInvalidationSubscriber
        implements CodexEventSubscriber<ContentItemRestoredEvent> {

    private static final Logger LOGGER =
            LoggerFactory.getLogger(ContentItemRestoredCacheInvalidationSubscriber.class);
     ...
     */
    public ContentItemRestoredCacheInvalidationSubscriber(
    ...
    public void handle(final ContentItemRestoredEvent event) {
        ...
        LOGGER.debug("Evicting content item cache entry on restore: site={} contentType={} key={}",
