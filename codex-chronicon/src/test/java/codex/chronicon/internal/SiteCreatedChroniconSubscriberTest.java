...
/**
 * Tests for {@link SiteCreatedChroniconSubscriber}.
 */
class SiteCreatedChroniconSubscriberTest {
    ...
    private SiteCreatedChroniconSubscriber subscriber;
    private SiteCreatedEvent event;

    @BeforeEach
    void setUp() {
        repository = new RecordingChroniconRepository();
        subscriber = new SiteCreatedChroniconSubscriber(repository);
        event = new SiteCreatedEvent(SITE_ID, SITE_KEY, ACTOR, NOW);
    }

    @Test
    void rejectsNullRepository() {
        assertThrows(NullPointerException.class, () -> new SiteCreatedChroniconSubscriber(null));
    }

    @Test
    void eventTypeIsSiteCreatedEvent() {
        assertEquals(SiteCreatedEvent.class, subscriber.eventType());
    }
