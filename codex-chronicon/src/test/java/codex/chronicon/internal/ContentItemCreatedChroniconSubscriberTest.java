...
/**
 * Tests for {@link ContentItemCreatedChroniconSubscriber}.
 */
class ContentItemCreatedChroniconSubscriberTest {
    ...
    private ContentItemCreatedChroniconSubscriber subscriber;
    private ContentItemCreatedEvent event;

    @BeforeEach
    void setUp() {
        repository = new RecordingChroniconRepository();
        subscriber = new ContentItemCreatedChroniconSubscriber(repository);
        event = new ContentItemCreatedEvent(ITEM_ID, SITE_KEY, CT_KEY, CT_VERSION_ID, ITEM_KEY, ACTOR, NOW);
    }

    @Test
    void rejectsNullRepository() {
        assertThrows(NullPointerException.class, () -> new ContentItemCreatedChroniconSubscriber(null));
    }

    @Test
    void eventTypeIsContentItemCreatedEvent() {
        assertEquals(ContentItemCreatedEvent.class, subscriber.eventType());
    }
