...
/**
 * Tests for {@link ContentItemArchivedChroniconSubscriber}.
 */
class ContentItemArchivedChroniconSubscriberTest {
    ...
    private ContentItemArchivedChroniconSubscriber subscriber;
    private ContentItemArchivedEvent event;

    @BeforeEach
    void setUp() {
        repository = new RecordingChroniconRepository();
        subscriber = new ContentItemArchivedChroniconSubscriber(repository);
        event = new ContentItemArchivedEvent(ITEM_ID, SITE_KEY, CT_KEY, CT_VERSION_ID, ITEM_KEY, ACTOR, NOW);
    }

    @Test
    void rejectsNullRepository() {
        assertThrows(NullPointerException.class, () -> new ContentItemArchivedChroniconSubscriber(null));
    }

    @Test
    void eventTypeIsContentItemArchivedEvent() {
        assertEquals(ContentItemArchivedEvent.class, subscriber.eventType());
    }
