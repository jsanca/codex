...
/**
 * Tests for {@link SiteSuspendedChroniconSubscriber}.
 */
class SiteSuspendedChroniconSubscriberTest {
    ...
    private SiteSuspendedChroniconSubscriber subscriber;
    private SiteSuspendedEvent event;

    @BeforeEach
    void setUp() {
        repository = new RecordingChroniconRepository();
        subscriber = new SiteSuspendedChroniconSubscriber(repository);
        event = new SiteSuspendedEvent(SITE_ID, SITE_KEY, ACTOR, NOW);
    }

    @Test
    void rejectsNullRepository() {
        assertThrows(NullPointerException.class, () -> new SiteSuspendedChroniconSubscriber(null));
    }

    @Test
    void eventTypeIsSiteSuspendedEvent() {
        assertEquals(SiteSuspendedEvent.class, subscriber.eventType());
    }
    ...
    @Test
    void savedRecordActionIsSuspended() {
        subscriber.handle(event);
        assertEquals(AuditAction.SUSPENDED, repository.savedRecords().get(0).action());
    }
