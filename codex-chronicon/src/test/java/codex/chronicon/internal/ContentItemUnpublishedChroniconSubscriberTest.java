...
/**
 * Tests for {@link ContentItemUnpublishedChroniconSubscriber}.
 */
class ContentItemUnpublishedChroniconSubscriberTest {
    ...
    private ContentItemUnpublishedChroniconSubscriber subscriber;
    private ContentItemUnpublishedEvent event;

    @BeforeEach
    void setUp() {
        repository = new RecordingChroniconRepository();
        subscriber = new ContentItemUnpublishedChroniconSubscriber(repository);
        event = new ContentItemUnpublishedEvent(ITEM_ID, SITE_KEY, CT_KEY, CT_VERSION_ID, ITEM_KEY, ACTOR, NOW);
    }

    @Test
    void rejectsNullRepository() {
        assertThrows(NullPointerException.class, () -> new ContentItemUnpublishedChroniconSubscriber(null));
    }
    ...
    @Test
    void savedRecordActionIsUnpublished() {
        subscriber.handle(event);
        assertEquals(AuditAction.UNPUBLISHED, repository.savedRecords().get(0).action());
    }
