...
/**
 * Tests for {@link ContentTypeActivatedChroniconSubscriber}.
 */
class ContentTypeActivatedChroniconSubscriberTest {
    ...
    private ContentTypeActivatedChroniconSubscriber subscriber;
    private ContentTypeActivatedEvent event;

    @BeforeEach
    void setUp() {
        repository = new RecordingChroniconRepository();
        subscriber = new ContentTypeActivatedChroniconSubscriber(repository);
        event = new ContentTypeActivatedEvent(
                CT_ID, SITE_KEY, CT_KEY, ContentTypeStatus.DRAFT, ContentTypeStatus.ACTIVE, ACTOR, NOW);
    }

    @Test
    void rejectsNullRepository() {
        assertThrows(NullPointerException.class, () -> new ContentTypeActivatedChroniconSubscriber(null));
    }

    @Test
    void eventTypeIsContentTypeActivatedEvent() {
        assertEquals(ContentTypeActivatedEvent.class, subscriber.eventType());
    }
    ...
    @Test
    void savedRecordActionIsActivated() {
        subscriber.handle(event);
        assertEquals(AuditAction.ACTIVATED, repository.savedRecords().get(0).action());
    }
