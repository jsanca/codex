...
/**
 * Tests for {@link ContentItemUpdatedChroniconSubscriber}.
 */
class ContentItemUpdatedChroniconSubscriberTest {
    ...
    private ContentItemUpdatedChroniconSubscriber subscriber;
    private ContentItemUpdatedEvent event;

    @BeforeEach
    void setUp() {
        repository = new RecordingChroniconRepository();
        subscriber = new ContentItemUpdatedChroniconSubscriber(repository);
        event = new ContentItemUpdatedEvent(ITEM_ID, SITE_KEY, CT_KEY, CT_VERSION_ID, ITEM_KEY, ACTOR, NOW);
    }

    @Test
    void rejectsNullRepository() {
        assertThrows(NullPointerException.class, () -> new ContentItemUpdatedChroniconSubscriber(null));
    }

    @Test
    void eventTypeIsContentItemUpdatedEvent() {
        assertEquals(ContentItemUpdatedEvent.class, subscriber.eventType());
    }
    ...
    @Test
    void savedRecordActionIsUpdated() {
        subscriber.handle(event);
        assertEquals(AuditAction.UPDATED, repository.savedRecords().get(0).action());
    }
